                            ...
                            <?php
                                ...
                                while($row = mysqli_fetch_assoc($result)){
                                    ...
                                    echo '<tr>
                                            ...
                                            <td>
                                                <img src="/OnlineOrdering/img/pizza-'.$pizzaId. '.jpg" alt="image for this item" width="150px" height="150px">
                                            </td>
                                            <td>
	  	...
	  	<form action="partials/_menuManage.php" method="post" enctype="multipart/form-data">
		    <div class="text-left my-2 row" style="border-bottom: 2px solid #dee2e6;">
				...
				<div class="form-group col-md-4">
					<img src="/OnlineOrdering/img/pizza-<?php echo $pizzaId; ?>.jpg" id="itemPhoto" name="itemPhoto" alt="item image" width="100" height="100">
				</div>
			</div>
		</form>
		<form action="partials/_menuManage.php" method="post">
			...
			<div class="text-left my-2 row">
				...
				<div class="form-group col-md-6">
                	...
                	<input class="form-control" id="stock" name="stock" value="<?php echo $pizzastock; ?>" type="number" min="1" required>
				</div>
				<div class="form-group col-md-6">
...
<div class="col-lg-12">
		<div class="row">
			<!-- FORM Panel -->
			<div class="col-md-4">
			<form action="partials/_menuManageAddon.php" method="post" enctype="multipart/form-data">
				<div class="card mb-3">
					<div class="card-header" style="background-color: rgb(111 202 203);">
						Create New Addons
				  	</div>
					<div class="card-body">
                            ...
                            <div class="form-group">
								<label class="control-label">Addon Price</label>
								<input type="number" class="form-control" name="addprice" required min="1">
							</div>	
							<div class="form-group">
								<label class="control-label">Addon Stock</label>
								<input type="number" class="form-control" name="addstocks" required min="1">
							</div>	
							<div class="form-group">
								<label class="control-label">Category: </label>
								<select name="addcategoryid" id="categoryId" class="custom-select browser-default" required>
								<option hidden disabled selected value>None</option>
                                <?php
                                    $catsql = "SELECT * FROM `categories`"; 
                                    $catresult = mysqli_query($conn, $catsql);
				...
				<div class="card">
					<div class="card-body">
						<table class="table table-bordered table-hover mb-0">
							...
							<tbody>
                            <?php
                                $sql = "SELECT * FROM `addonns`";
                                $result = mysqli_query($conn, $sql);
                                while($row = mysqli_fetch_assoc($result)){
                                    $addid = $row['addid'];
                                    $addname = $row['addname'];
                                    $addprice = $row['addprice'];
									$pizzaCategorieId = $row['addcategoryid'];
									$addstock = $row['addstocks'];

                                    echo '<tr>
                                            <td class="text-center">' .$pizzaCategorieId. '</td>
                                            <td>
                                                <img src="/OnlineOrdering/img/addon-'.$addid. '.jpg" alt="image for this item" width="150px" height="150px">
                                            </td>
                                            <td>
                                                <p>Name : <b>' .$addname. '</b></p>
                                               
                                                <p>Price : <b>' .$addprice. '</b></p>
												  <p>stock : <b>' .$addstock. '</b></p>

                                            </td>
                                            <td class="text-center">
												<div class="row mx-auto" style="width:112px">
													<button class="btn btn-sm btn-primary" type="button" data-toggle="modal" data-target="#updateItem' .$addid. '">Edit</button>
													<form action="partials/_menuManageAddon.php" method="POST">
														<button name="removeItem" class="btn btn-sm btn-danger" style="margin-left:9px;">Delete</button>
...
<?php 
    $pizzasql = "SELECT * FROM `addonns`";
    $pizzaResult = mysqli_query($conn, $pizzasql);
    while($pizzaRow = mysqli_fetch_assoc($pizzaResult)){
        $pizzaId = $pizzaRow['addid'];
        $pizzaName = $pizzaRow['addname'];
        $pizzaPrice = $pizzaRow['addprice'];
        $pizzaCategorieId = $pizzaRow['addcategoryid'];
		$pizzastock = $pizzaRow['addstocks'];
?>
      ...
      <div class="modal-body">
	  	<form action="partials/_menuManageAddon.php" method="post" enctype="multipart/form-data">
		    <div class="text-left my-2 row" style="border-bottom: 2px solid #dee2e6;">
				...
				<div class="form-group col-md-4">
					<img src="/OnlineOrdering/img/addon-<?php echo $pizzaId; ?>.jpg" id="itemPhoto" name="itemPhoto" alt="item image" width="100" height="100">
				</div>
			</div>
		</form>
		<form action="partials/_menuManageAddon.php" method="post">
            <div class="text-left my-2">
                <b><label for="name">Name</label></b>
                <input class="form-control" id="addname" name="addname" value="<?php echo $addname; ?>" type="text" required>
            </div>
			
			<div class="text-left my-2 row">
				<div class="form-group col-md-6">
                	<b><label for="price">Price</label></b>
                	<input class="form-control" id="addprice" name="addprice" value="<?php echo $addprice; ?>" type="number" min="1" required>
				</div>
				<div class="form-group col-md-6">
                	<b><label for="stock">stock</label></b>
                	<input class="form-control" id="stock" name="addstocks" value="<?php echo $addstocks; ?>" type="number" min="1" required>
				</div>
				<div class="form-group col-md-6">
					<b><label for="catId">Category Id</label></b>
                	<input class="form-control" id="catId" name="addcategoryid" value="<?php echo $addcategorieid; ?>" type="number" min="1" required>
				</div>
            </div>
            
            <input type="hidden" id="pizzaId" name="addid" value="<?php echo $addid; ?>">
            <button type="submit" class="btn btn-success" name="updateItem">Update</button>
        </form>
      </div>
    </div>
