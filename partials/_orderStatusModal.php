...
if (isset($_POST['cancelOrder'])) {
    ...
    $checkStatusSql = "SELECT orderStatus FROM orders WHERE orderId=? AND userId=?";
    $stmt = mysqli_prepare($conn, $checkStatusSql);
    mysqli_stmt_bind_param($stmt, "ii", $orderId, $userId);
    mysqli_stmt_execute($stmt);
    $result = mysqli_stmt_get_result($stmt);
    $row = mysqli_fetch_assoc($result);
    
    // Only allow cancellation if status is 0 or 1
    if($row && $row['orderStatus'] <= 0) {
        $cancelSql = "UPDATE `orders` SET `orderStatus`=6 WHERE `orderId`=? AND `userId`=?";
        $stmt = mysqli_prepare($conn, $cancelSql);
        mysqli_stmt_bind_param($stmt, "ii", $orderId, $userId);
        mysqli_stmt_execute($stmt);
    }
}
...
$stmt = mysqli_prepare($conn, $statusmodalsql);
mysqli_stmt_bind_param($stmt, "i", $userId);
mysqli_stmt_execute($stmt);
$statusmodalresult = mysqli_stmt_get_result($stmt);
...
while($statusmodalrow = mysqli_fetch_assoc($statusmodalresult)){
    ...
    $tstatus = isset($statusMessages[$status]) ? $statusMessages[$status] : "Unknown Status";
    ...
    // Get delivery details if order is in progress
    if($status >= 1 && $status <= 4) {
        $deliveryDetailSql = "SELECT * FROM `deliverydetails` WHERE `orderId`= ?";
        $stmt = mysqli_prepare($conn, $deliveryDetailSql);
        mysqli_stmt_bind_param($stmt, "i", $orderid);
        mysqli_stmt_execute($stmt);
        $deliveryDetailResult = mysqli_stmt_get_result($stmt);
...
<?php
// Only show cancel button if order is not yet being prepared (status 0 or 1)
if($status <= 1): ?>
    <form method="POST">
        <input type="hidden" name="orderId" value="<?php echo $orderid; ?>">
        <button type="submit" name="cancelOrder" class="btn btn-danger" 
